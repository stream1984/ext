    ...
    try {
      ...
      return curator.getChildren().forPath(mapPath).stream().anyMatch(e -> {
        try {
          ...
          KeyValue<K, V> keyValue = (KeyValue<K, V>) asObject(bytes);
  ...
  public V get(Object key) {
    try {
      ...
      if (null == curator.checkExists().forPath(keyPath)) {
        ...
        KeyValue<K, V> keyValue = (KeyValue) asObject(curator.getData().forPath(keyPath));
  ...
  public Set<K> keySet() {
    try {
      ...
      return curator.getChildren().forPath(mapPath).stream().map(e -> {
        try {
          KeyValue<K, V> keyValue = (KeyValue) asObject(curator.getData().forPath(keyPath((K) e)));
